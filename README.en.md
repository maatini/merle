...
## Professional Status — v0.7.1

| Area                 | Status   | Notes                                                                                                                                                         |
| -------------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `merle-core`         | ✅ 0.7.1 | `BaseBot`, `BaseTask`, retry (tenacity), observability (OTel, extra), secrets (Azure, extra), NATS client (extra), Playwright wrapper (Chromium + Lightpanda) |
...
**High-level (current = v0.7.1):**

- **Foundation (complete through v0.5.x):** Core library, Copier template, CLI, CI/CD, governance, `.opencode/` hybrid agent
- **DX & Hardening (through v0.7.1):** Gold examples (incl. UiPath hybrid), coverage + security hard gates, monorepo/standalone Docker deploy path
